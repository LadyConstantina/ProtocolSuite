        ...
        for resp in resp_services:
            if resp == 'me':
                ...
                threading.Thread(target=FTPServer(
                    self.ftp_cmd_port, self.ftp_data_port, r"C:\Users\User\Desktop\PR3\PR3_LAB-main\100%\follower2\media"
                ).run).start()
                ftp_ports.append({
...
class Follower_Server(Base_Server):
    ...
    def add_element(self, dic):
        ...
        self.Table[index] = dic
        threading.Thread(target=FTPServer(
            self.ftp_cmd_port, self.ftp_data_port, r"D:\70v2\follower2\media"
        ).run).start()
        return {
                   "ftp_cmd_port" : self.ftp_cmd_port,
                   "ftp_data_port" : self.ftp_data_port,
